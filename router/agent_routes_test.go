...
func TestAgentRoutesAuthenticateOwnerAndReturnStringMoney(t *testing.T) {
	...
	require.NoError(t, db.AutoMigrate(
		&model.User{}, &model.AgentAccount{}, &model.AgentCreditLog{},
	...
	engine := gin.New()
	engine.Use(sessions.Sessions("session", cookie.NewStore([]byte("agent-route-test"))))
	engine.GET("/test/login", func(c *gin.Context) {
		session := sessions.Default(c)
		session.Set("username", "agent-route")
	...
	doRequest := func(method string, path string, body string) *httptest.ResponseRecorder {
		...
		request.Header.Set("New-Api-User", "51")
		if body != "" {
