...
func TestAgentCustomerRoutesEnforceOwnershipAndAllowDisabledReads(t *testing.T) {
	...
	now := time.Now().Unix()
	users := []model.User{
		{Id: 101, Username: "route-agent", AffCode: "route-agent-aff", Role: common.RoleCommonUser, Status: common.UserStatusEnabled},
		{Id: 102, Username: "route-disabled", AffCode: "route-disabled-aff", Role: common.RoleCommonUser, Status: common.UserStatusEnabled},
	...
	usersByID := map[int]model.User{101: users[0], 102: users[1]}
	engine := gin.New()
	engine.Use(sessions.Sessions("session", cookie.NewStore([]byte("agent-customer-route-test"))))
	...
	request := func(userID int, target string) *httptest.ResponseRecorder {
		...
		engine.ServeHTTP(login, httptest.NewRequest(http.MethodGet, "/test/login/"+strconv.Itoa(userID), nil))
