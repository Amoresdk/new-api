...
func TestUserRedeemRouteAuthTypedDeliveryAndCriticalRateLimit(t *testing.T) {
	...
	require.NoError(t, db.AutoMigrate(
		&model.User{}, &model.Log{}, &model.SubscriptionPlan{}, &model.UserSubscription{},
	...
	engine := gin.New()
	engine.Use(sessions.Sessions("session", cookie.NewStore([]byte("user-redemption-route-test"))))
	...
	doRedeem := func(key string) *httptest.ResponseRecorder {
		...
		request.Header.Set("New-Api-User", "8201")
