...
func channelHasSensitiveChanges(channel *PatchChannel, origin *model.Channel, requestData map[string]any) bool {
	...
	// actual_base_url: nil means "leave unchanged" (non-root requests are nil-ed
	// out in UpdateChannel before this check), so only a non-nil differing value
	// counts as a sensitive change.
	if _, ok := requestData["actual_base_url"]; ok && channel.ActualBaseURL != nil && !equalStringPtr(channel.ActualBaseURL, origin.ActualBaseURL) {
		return true
	}
